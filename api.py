...
async def root():
    ...
    return JSONResponse(content={
        ...
        "endpoints": {
            ...
            "get_ticker_range": "/stock/{ticker}/range?start=2024-01-01&end=2024-01-31",
...
async def get_multiple_tickers(
    ...
    if fields:
        ...
        selected_fields = [f for f in field_list if f in valid_fields]
        if not selected_fields:
            selected_fields = ['ticker', 'date', 'close', 'volume']
    else:
        selected_fields = ['ticker', 'date', 'close', 'volume']
    
    if 'ticker' not in selected_fields:
        selected_fields.insert(0, 'ticker')
    if 'date' not in selected_fields:
